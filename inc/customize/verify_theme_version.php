		...
		$request = wp_remote_get('https://vwthemes.com/version/themes/home?theme_name='.$theme_name);
			if ( ! is_wp_error( $request ) || wp_remote_retrieve_response_code( $request ) === 200) {
				...
				if($version != $data) {
					...
					<p>There is an update available for the <strong>' . esc_html($theme_name) . '</strong> theme. <a href="https://www.vwthemes.com/my-account/" target="_BLANK">Update Now</a></p>
					</div>';
				}
			} ?>
	<?php } ?>