 ...
 */
function vw_physiotherapy_pro_logo_customize_register( $wp_customize ) {
	...
	$wp_customize->add_control( 'logo_size', array(
		'label'       => esc_html__( 'Logo Size','vw-appointment-pro' ),
...
add_action( 'customize_register', 'vw_physiotherapy_pro_logo_customize_register' );

/**
 * Add support for logo resizing by filtering `get_custom_logo`
 */
function vw_physiotherapy_pro_customize_logo_resize( $html ) {
	...
	if ( is_numeric( $size ) && is_numeric( $custom_logo_id ) ) {
		...
		if ( $logo['width'] >= $logo['height'] ) {
			$output = vw_physiotherapy_pro_min_max( $logo['height'], $logo['width'], $max['height'], $max['width'], $size, $min );
			$img = array(
				'height'	=> $output['short'],
				'width'		=> $output['long']
			);
		// portrait
			...
			$output = vw_physiotherapy_pro_min_max( $logo['width'], $logo['height'], $max['width'], $max['height'], $size, $min );
			$img = array(
				'height'	=> $output['long'],
				'width'		=> $output['short']
			);
...
add_filter( 'get_custom_logo', 'vw_physiotherapy_pro_customize_logo_resize' );

/* Helper function to determine the max size of the logo */
function vw_physiotherapy_pro_min_max( $short, $long, $short_max, $long_max, $percent, $min ){
	$ratio = ( $long / $short );
 ...
 */
function vw_physiotherapy_pro_customize_preview_js() {
...
add_action( 'customize_preview_init', 'vw_physiotherapy_pro_customize_preview_js' );

/**
 * JS handlers for Customizer Controls
 */
function vw_physiotherapy_pro_customize_controls_js() {
...
add_action( 'customize_controls_enqueue_scripts', 'vw_physiotherapy_pro_customize_controls_js' );

/**
 * Testing function to remove logo_size theme mod
 */
function vw_physiotherapy_pro_remove_theme_mod() {
	if ( isset( $_GET['remove_logo_size'] ) && 'true' == $_GET['remove_logo_size'] ){
		set_theme_mod( 'logo_size', '' );
	}
}
add_action( 'wp_loaded', 'vw_physiotherapy_pro_remove_theme_mod' );