    ...
    function PluginManager(){
                ...
                if(typeof response.url != 'undefined'){
                    ...
                    if(response.hash == current_item_hash){
                        ...
                        jQuery.post(response.url, response, function(response2) {
                            process_current();
                            $current_node.find('.wizard-plugin-status').text(response.message + vw_appointment_pro_whizzie_params.verify_text);
                        }).fail(ajax_callback);
        ...
        function process_current() {
            if(current_item){
                ...
                jQuery.post(vw_appointment_pro_whizzie_params.ajaxurl, {
                    action: 'setup_plugins',
                    wpnonce: vw_appointment_pro_whizzie_params.wpnonce,
                    slug: current_item
	...
	function WidgetManager(demo_type) {
        ...
        if(demo_type == 'builder'){
    		...
    		function import_widgets(){
                jQuery.post(
    				vw_appointment_pro_whizzie_params.ajaxurl,
    				{
...
jQuery(document).ready(function(){
    ...
    jQuery('#vw_appointment_pro_license_form button[id="change--key"]').on('click', function() {
      var $vw_appointment_pro_license_form = jQuery('#vw_appointment_pro_license_form');
      $vw_appointment_pro_license_form.find('input[name="vw_appointment_pro_license_key"]').val('');
      $vw_appointment_pro_license_form.find('input[name="vw_appointment_pro_license_key"]').attr('disabled', false);
      $vw_appointment_pro_license_form.find('button[type="submit"]').val('');
      $vw_appointment_pro_license_form.find('button[type="submit"]').attr('disabled', false);
      $vw_appointment_pro_license_form.find('button[type="submit"]').text('Activate');
      jQuery('#start-now-next').hide();
      jQuery(this).remove();
    });

    jQuery('form#vw_appointment_pro_license_form').on('submit', function(e) {
        jQuery('.theme_activation_spinner').show();
      e.preventDefault();
      var key_to_send = jQuery('form#vw_appointment_pro_license_form').serializeArray()[0].value;
      ...
      if (key_to_send == "") {
        ...
        jQuery.post(
          vw_appointment_pro_whizzie_params.ajaxurl,
          {
            action: 'wz_activate_vw_appointment_pro',
            wpnonce: vw_appointment_pro_whizzie_params.wpnonce,
            vw_appointment_pro_license_key: key_to_send
          },
                    ...
                    if (status == 'success') {
                        if (data.status) {
                           location.reload(true);
                            ...
                            jQuery('form#vw_appointment_pro_license_form button[type="submit"]').css("background-color:#0a9d2c");
                            jQuery('form#vw_appointment_pro_license_form button[type="submit"]').text('Activated');
                            jQuery('form#vw_appointment_pro_license_form button[type="submit"]').attr('disabled', 'disabled');
